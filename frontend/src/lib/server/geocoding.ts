...
import { MAPBOX_TOKEN, OPENAI_API_KEY, OPENAI_MODEL } from "$lib/server/env";
...
export const KNOWN_LOCATIONS: Record<string, { lat: number; lng: number; country: string }> = {
	...
	Malappuram: { lat: 11.051, lng: 76.0711, country: "India" },
	...
	Bangladesh: { lat: 23.685, lng: 90.3563, country: "Bangladesh" },
	...
	LOCATION_PATTERNS.forEach((pattern) => {
		...
		while ((match = regex.exec(text)) !== null) {
			...
			if (candidate && !LOCATION_BLACKLIST.has(candidate)) {
				if (candidate.length >= 3 && !(candidate.length <= 4 && candidate === candidate.toUpperCase())) {
...
async function extractLocationWithAI(title: string, regexCandidates: string[]): Promise<string | null> {
	if (!openai) {
		return null;
	}
	...
	try {
		const response = await openai.chat.completions.create({
			...
			messages: [
				{
					role: "system",
					content: `You are a location extraction assistant. Given a news headline and optional location candidates, identify the MOST SPECIFIC geographic location mentioned in the content where the event is happening.

CRITICAL PRIORITY:
- Prefer CITY or TOWN names over broader regions or countries
- Extract the location from the NEWS CONTENT itself, NOT from where the news source is based
- Look for specific places mentioned like "in [city]", "at [location]", etc.
- Only return broader regions/countries if NO specific city/town is mentioned

Respond with ONLY the most specific location name, nothing else. If no clear location can be determined, respond with "UNKNOWN".`
				},
				{
					role: "user",
					content: `Headline: "${title}"${candidatesText}\n\nWhat is the most specific location mentioned in this news (preferring city-level detail)?`
				}
			],
			max_tokens: 50,
			temperature: 0
...
export async function geocodeLocationsFromText(
	...
	for (const candidate of regexCandidates) {
		if (!locationNames.some((loc) => loc.toLowerCase() === candidate.toLowerCase())) {
