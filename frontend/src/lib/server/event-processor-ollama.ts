/**
 * Event processor using local Ollama models
 *
 * Setup:
...
import { OLLAMA_BASE_URL, OLLAMA_MODEL } from './env';
...
/**
 * Normalize raw input to a standard format
 */
...
/**
 * Extract structured event data using Ollama
 */
...
Extract structured intelligence data from the provided content. Be precise with locations - extract the specific city/town where events are occurring.${keywordContext}
...
You MUST respond with ONLY a valid JSON object, no other text. Use this exact format:
{
  ...
  "timestamp": "ISO 8601 date or null",
  "is_relevant": true
}
...
JSON:`;
	...
	try {
		...
		if (!parsed.title || !parsed.threat_level) {
			return null;
		}
...
/**
 * Process a raw input using Ollama
 */
export async function processRawInput(
	...
	if (ollamaAvailable) {
		extracted = await extractWithOllama(topic, normalized.title, normalized.content, keywords);
	}
...
/**
 * Process multiple raw inputs using Ollama
 */
