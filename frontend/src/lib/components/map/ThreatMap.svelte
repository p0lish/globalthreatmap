<script lang="ts">
	...
	function initMap() {
		...
		map.on('mouseenter', 'unclustered-point', () => map && (map.getCanvas().style.cursor = 'pointer'));
		map.on('mouseleave', 'unclustered-point', () => map && (map.getCanvas().style.cursor = ''));
	}
	...
	function updateEntityLocations() {
		...
		const source = map.getSource('entity-locations') as mapboxgl.GeoJSONSource;
	...
	async function handleMapClick(e: mapboxgl.MapMouseEvent) {
		...
		const features = map.queryRenderedFeatures(e.point, {
			layers: ['clusters', 'unclustered-point', 'entity-locations', 'hospitals-circle']
		});
	...
	$effect(() => {
		if (map) {
			if (map.getLayer('hospitals-circle')) {
				map.setLayoutProperty('hospitals-circle', 'visibility', $showHospitals ? 'visible' : 'none');
			}
			if (map.getLayer('hospitals-labels')) {
				map.setLayoutProperty('hospitals-labels', 'visibility', $showHospitals ? 'visible' : 'none');
			}
		}
	});
...
{#if !MAPBOX_TOKEN}
	<div class="flex h-full w-full items-center justify-center bg-card">
		<div class="text-center">
			<p class="text-lg font-semibold text-foreground">Mapbox Token Required</p>
			<p class="text-sm text-muted-foreground">Please add VITE_MAPBOX_TOKEN to your .env file</p>
		</div>
	</div>
{:else}
