...
} from "$lib/server/valyu";
import { isSelfHostedMode } from "$lib/server/app-mode";
import { geocodeLocationsFromText } from "$lib/server/geocoding";
import type { EntityProfile, GeoLocation } from "$lib/types";
...
export const GET: RequestHandler = async ({ url }) => {
	...
	try {
		...
		return json({ entity: profile });
	} catch (error) {
		console.error("Error researching entity:", error);
		return json({ error: "Failed to research entity" }, { status: 500 });
	}
};
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		return json({ entity: profile, deliverables, pdfUrl });
	} catch (error) {
		console.error("Error researching entity:", error);
		return json({ error: "Failed to research entity" }, { status: 500 });
	}
