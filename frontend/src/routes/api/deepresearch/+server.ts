...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		if (!selfHosted && accessToken) {
			...
			return json({ taskId: result.taskId, status: "queued" });
		}
...
- Timeline of significant events`,
		...
		if (!task.success || !task.deepresearch_id) {
			...
			return json({ error: task.error || "Failed to create research task" }, { status: 500 });
		}
		...
		return json({ error: "Failed to create research task" }, { status: 500 });
	}
};
